...
def run_installation():
    ...
    for cmd in commands:
        ...
        output += f"<br>{cmd}: {result.stdout or result.stderr}"
...
def network():
    ...
    if request.method == 'POST':
        ...
        selected_scans = request.form.getlist('scan_options')  # Get checked checkboxes
        
        if not selected_scans:
            return render_template('net.html', error="No scan options selected!")

        results, analysis_report = run_nmap_scan(target, selected_scans)
...
def web():
    ...
    if request.method == 'POST':
        ...
        results, analysis_report = run_nikto_scan(target, selected_scans)
